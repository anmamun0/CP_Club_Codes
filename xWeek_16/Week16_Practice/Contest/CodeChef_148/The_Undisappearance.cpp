#include <bits/stdc++.h>
#include<ext/pb_ds/assoc_container.hpp>
#include<ext/pb_ds/tree_policy.hpp>
using namespace __gnu_pbds;
using namespace std;
template <typename T> 
using pbds = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>;
typedef long long ll;
typedef pair<int, int> couple;
#define  str string
#define  X first
#define  Y second
#define  pb push_back
#define  all(x) x.begin(),x.end() 
#define  Public int T; input(T);while(T--)
#define  checkmate return  
#define  __stop print('\n');
#define Run(i, x, y) for (int _s=x, _e=y, _in=(y>x)?1:-1, i=_s;((_in==1)?(i<=_e):(i>=_e)); i+=_in)
template <typename... DataIn> void input(DataIn&... val){(cin >> ... >> val);}
template <typename... DataOut> void print(const DataOut&... val){((cout << val), ...);} 
namespace AN{inline void yes(){cout<<"YES\n";}void no(){cout<<"NO\n";}bool isEven(auto x){return (x&1==0);} bool isPlus(auto x){return (x>=0);} 
    int len(auto x){return (int)(x).size();} template <typename T> void input(vector<T> &v) { for (auto &c : v) cin >> c; } 
    template <typename T> void print(vector<T> v){ for(auto c:v)cout<<c<<" ";}char ln = '\n';}
using namespace AN;

int main()
{
  /* _Coder   : anmamun0
     _Created : 21 August 2024 ||  21:33:13
     _File    : The_Undisappearance.cpp 
    بِسْمِ اللهِ الرَّحْمٰنِ الرَّحِيْمِ */
    ios::sync_with_stdio(false);
    cin.tie(NULL);

    Public{
        int n; input(n);
        vector<int> v(n); input(v);

        int one = 0, two = 0, three = 0, cnt = 0; ;
        for(auto c:v)  (c==1)?one++:((c==2)?two++:three++);  
        vector<vector<int>> y(n, vector<int>(3, 0));
        if(v[0]==1)y[0][0]++;
        else if(v[0]==2) y[0][1]++;
        else y[0][2]++;

        for(int i = 1; i<n; i++)
        {
            if(v[i]==1)y[i][0]++;
            else if(v[i]==2) y[i][1]++;
            else y[i][2]++;

            y[i][0] += y[i - 1][0];
            y[i][1] += y[i - 1][1];
            y[i][2] += y[i - 1][2];
        }
        
        for(int i = 0; i<n; i++)
        {
            for(int j = i; j<n; j++)
            {
                int a = y[j][0],b = y[j][1], c = y[j][2];

                if(i!=0)
                {
                    a -= y[i - 1][0];
                    b -= y[i - 1][1];
                    c -= y[i - 1][2];
                }

                int _1 = one, _2 = two, _3 = three;
                _1 -= a;
                _2 -= b;
                _3 -= c;

                _1 += c;
                _2 += a;
                _3 += b;
                if(_1 >0 and _2>0 and _3 >0) cnt++;
            }
        }
 
        print(cnt, ln);
    }

    checkmate 0;
}