...
{
  /* _Coder   : anmamun0
     _Created : 16 September 2024 ||  22:35:06
     _File    : C_AB_Flipping.cpp 
    ...
    testCase
    {
        ...
        if(!s.empty() and s[0]=='B')
        {
            int x = s.find_first_not_of('B');
            s.erase(0, x);
        }  
 
        if(!s.empty() and s.back()=='A')
        {
            int x = s.find_last_not_of('A');
            s.erase(x + 1);
        }
 
        print((s.empty()) ? 0 : s.size() - 1, ln); 
    }
