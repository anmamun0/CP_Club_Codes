...
{
  /* _Coder   : anmamun0
     _Created : 14 September 2024 ||  21:00:11
     _File    : B_1_The_Strict_Teacher_Easy_Version.cpp 
    ...
    testCase
    {
        ...
        vector<int> T(m);  input(T);  
        sort(begin(T), end(T));

        while(q--)
        {
            int pos,ans = 0; input(pos); 
            if(pos< T[0]) ans = T.front() - 1;  
            else if(pos>T.back()) ans = n - T.back();  
            else 
            {
                auto one = upper_bound(begin(T), end(T), pos);
                auto two = prev(one); 
                int mid = (*one+*two)/2;
                ans = min(abs(mid - *one), abs(mid - *two));
            }
            cout << ans << endl;
        }
